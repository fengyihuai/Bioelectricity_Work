...
for fi=1:length(freqs2use)
    ...
    % phase angles form channel 2 via convolution
...
%     time_window_idx = round(100/(1000/EEG.srate)); % set 100 for figure 3c
    time_window_idx = round(300/(1000/EEG.srate)); % set 300 for figure 3d
...
for triali=1:EEG.trials
    ...
    % You can also replace this function with your preferred filter method
    % (chapter 14).
...
% initialize
spectchoer = zeros(length(freqs2use), length(times2save));
...
for fi=1:length(freqs2use)
    ...
    % alternative notation for the same procedure, using the Euler-like
    % expression: Me^ik
...
phases = phases - pi/2;
polar([phases; phases], repmat([0 1]', n, 1), 'k');
pli = abs(mean(exp(sign(imag(exp(1i*phases))))));
ispc = abs(mean(exp(1i*phases)));
title([ 'PLI=' num2str(pli) ', ISPC=' num2str(ispc) ])
...
timewindow = linspace(1.5, 3, length(freqs2use));
n_wavelet = length(time);
n_data = EEG.pnts*EEG.trials;
n_convolution = n_wavelet + n_data - 1;
...
for fi = 1:length(freqs2use)
    ...
    % debiased weighted phase-lag index (short, as implemented in fieldtrip)
    imagsum = sum(cdi, 2);
    ...
    % compute time window in indices for this frequency
    time_window_idx = round((1000/freqs2use(fi))*timewindow(fi)/(1000/EEG.srate));
    % 1000/freqs2use(fi) - signal period, 1000/EEG.srate - data interval,
    % timewindow(fi) - number of cycles (half), time_window_idx - data
    % idxes of one cycle (half)
...
for idx = animate_start:animate_stop
    ...
    % update angles
    for i=1:length(hpol) % length(hpol)==time_windows_idx
...
for i=1:2
    ...
    set(h, 'linestyle', 'none') % hide lines
